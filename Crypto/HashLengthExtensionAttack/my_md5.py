...
if __name__ == '__main__':
    def usage():
        print "usage:\n"+"md5 [option]...[string or file]\n"+"-s,--string   compute md5 with this string\n"+"-f --file   compute md5 with the content of this file"
    if len(sys.argv)==1:
        usage()
    else:   
        if sys.argv[1]=="-h" or sys.argv[1]=="--help":
            ...
            print "msg>>%s\n"%sys.argv[2],"md5>>%s\n"%run_md5(readyMsg=deal_rawInputMsg(sys.argv[2]))
        elif sys.argv[1]=="-f" or sys.argv[1]=="--file":
            intput_msg = open(sys.argv[2],'r').read()
            print "md5>>%s\n"%run_md5(readyMsg=deal_rawInputMsg(intput_msg))
        else:
            print "option error!\n"+"please input -h or --help for usage info!"